...
class EMS_GT {
public:
	EMS_GT(const DataSetParams *ds, const Config *c, string plantedMotif): 
		ds(ds), config(c)
	{
    	currentNeighborhood = new int[config->numberOfCandidateMotifsRows];
    	this->plantedMotif = plantedMotif;
    	this->encodedMotif = encode (plantedMotif);
	}
	        ...
	        for (int j=0; j < 32; j++) {
	            if ((value & 1) != 0) {
	                ...
	                if (numberOfBitsPerBlock[(int)(i/32)] <= 1) {
	                	if (candidate == encodedMotif && isMotif(candidate)) {
	                ...
	                // if this is the first lmer in the block to be tested
		                ...
		                if (candidate == encodedMotif && isMotifInitializeFilter(candidate)) {
		                    foundMotifs += " " + decode(candidate, ds->lengthOfMotif);
		                    numMotifs++;
		                }
	                }
	                ...
	                // else if there exists a filtered lmers and a sequences, use that
	                	...
	                	if (candidate == encodedMotif && isMotifUseFilter(candidate)) {
		                    foundMotifs += " " + decode(candidate, ds->lengthOfMotif);
		                    numMotifs++;
		                }
	                }
